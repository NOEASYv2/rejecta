...
void Visuals::RenderBox()
{
	...
	case 3:
		Vector* boxVectors = GetBox(ESP_ctx.player);
			...
			Math::WorldToScreen(boxVectors[7], boxVectors7))
		{
			...
			{
				g_VGuiSurface->DrawSetColor(ESP_ctx.clr);
				g_VGuiSurface->DrawLine(lines[i][0].x, lines[i][0].y, lines[i][1].x, lines[i][1].y);
				g_VGuiSurface->DrawSetColor(col_black);
				g_VGuiSurface->DrawLine(lines[i][0].x - 1, lines[i][0].y - 1, lines[i][1].x - 1, lines[i][1].y - 1);
				g_VGuiSurface->DrawSetColor(col_black);
				g_VGuiSurface->DrawLine(lines[i][0].x + 1, lines[i][0].y + 1, lines[i][1].x + 1, lines[i][1].y + 1);
			}
		}
...
void Visuals::Esp_Ammo()
{
	...
	if (activity == 967 && animLayer->m_flWeight != 0.f)
	{
		...
		height = (ESP_ctx.bbox.right - ESP_ctx.bbox.left) + (ammo / iClipMax);
...
void Visuals::Esp_LBYTimer() //gonna rework this soon
{
	static float LBY[64];
	...
	Color color = Color(playerTeam ? g_Options.esp_player_lbytimer_color_t : g_Options.esp_player_lbytimer_color_ct);

	float lby = ESP_ctx.player->m_flLowerBodyYawTarget();

	int Index = ESP_ctx.player->EntIndex();

	if (lby != LBY[Index] || (ESP_ctx.player->m_vecVelocity().Length2D() > 0.1f && ESP_ctx.player->m_fFlags() & FL_ONGROUND))
	{
		LBY[Index] = lby;
		time[Index] = g_GlobalVars->curtime;
	}

	wchar_t buf[80];

	swprintf(buf, L"%.2f", std::fmaxf(1.1f - g_GlobalVars->curtime + time[Index], 0.f));
...
void Visuals::RenderHud()
{
	...
	if (!g_EngineClient->IsInGame() || !g_EngineClient->IsConnected() || !pLocal->IsAlive())
		return;
	...
	DrawString(hud_font, 10, height - offsetH - fontT, Global::LBY ? Color(255, 40, 40) : Color(40, 255, 40), FONT_LEFT, "LBY");

	DrawString(hud_font, 10, height - offsetH - 2 * fontT, Global::lagcomp ? Color(255, 40, 40) : Color(40, 255, 40), FONT_LEFT, "LC");
}
...
void Visuals::Esp_Skeleton()
{
	...
	else if (g_Options.esp_player_skeleton_type == 1) //uc pasta https://www.unknowncheats.me/forum/counterstrike-global-offensive/261581-appealing-bone-esp.html
	{
		studiohdr_t* pStudioModel = g_MdlInfo->GetStudiomodel(ESP_ctx.player->GetModel());
...
{
	//i think something was here but looks like antipaste fuck uc kiddos
...
void Visuals::HitboxESP(matrix3x4_t* matrix, Vector bbmin, Vector bbmax, int bone, Color color)
{
	...
	Vector pointsTransformed[99999];
...
void Visuals::ManualAA() 
{
	...
	int hx, hy;
	hx = GetViewport().right / 2;
	hy = GetViewport().bottom / 2;

	Vertex_t vertsR[3] = 
	{
		Vector2D(hx, hy), Vector2D(20 + hx, 5 + hy), Vector2D(20 + hx, -5 + hy) //half of viewport -10 (-10 further for centery point
	};

	Vertex_t vertsL[3] = 
	{
		Vector2D(hx), Vector2D(hx - 20, hy + 5), Vector2D(hx - 20, hy - 5) //half of viewport +10 (+10 further for centery point
	};

	Vertex_t vertsB[3] =
	{
		Vector2D(hx, hy + 30), Vector2D(hx - 5, hy + 20), Vector2D(hx + 5, hy + 20) //half of viewport +10 (+10 further for centery point
	};

	Color cL = (AntiAim::Get().side < -0.5) ? Color(g_Options.esp_manual_aa_indicator_color) : Color(255, 255, 255, 255);
	Color cR = (AntiAim::Get().side > 0.5) ? Color(g_Options.esp_manual_aa_indicator_color) : Color(255, 255, 255, 255);

	PolyLine(3, vertsR, cR);
	PolyLine(3, vertsL, cL);
...
void Visuals::Draw3DBox(Vector* points, Color color)
{
	Vector boxVectors0, boxVectors1, boxVectors2, boxVectors3,
		boxVectors4, boxVectors5, boxVectors6, boxVectors7;


	if (Math::WorldToScreen(points[0], boxVectors0) &&
		Math::WorldToScreen(points[1], boxVectors1) &&
		Math::WorldToScreen(points[2], boxVectors2) &&
		Math::WorldToScreen(points[3], boxVectors3) &&
		Math::WorldToScreen(points[4], boxVectors4) &&
		Math::WorldToScreen(points[5], boxVectors5) &&
		Math::WorldToScreen(points[6], boxVectors6) &&
		Math::WorldToScreen(points[7], boxVectors7)) {
		...
		{
			g_VGuiSurface->DrawSetColor(color);
			g_VGuiSurface->DrawLine(static_cast<int>(lines[i][0].x), static_cast<int>(lines[i][0].y), static_cast<int>(lines[i][1].x), static_cast<int>(lines[i][1].y));
		}
	}
}
