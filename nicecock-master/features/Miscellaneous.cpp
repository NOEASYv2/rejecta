...
void Miscellaneous::Fakelag()
{
	...
	int smartchoke = std::min<int>(g_Options.misc_fakelag_adaptive ? static_cast<int>(std::ceilf(64 / (g_LocalPlayer->m_vecVelocity().Length() * g_GlobalVars->interval_per_tick))) : g_Options.misc_fakelag_smart, 14);

	if (g_Options.misc_fakelag_adaptive && groundchoke > 14) return;  //this has gotten more ugly
	if (g_Options.misc_fakelag_adaptive && airchoke > 14) return;	  //than my maths exam lol
	if (g_Options.misc_fakelag_adaptive && movechoke > 14) return;	  //
	if (g_Options.misc_fakelag_adaptive && edgechoke > 14) return;	  //
	if (g_Options.misc_fakelag_adaptive && smartchoke > 14) return;	  //
	...
	else if (smart) { tochoke = smartchoke; }
