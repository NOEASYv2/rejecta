...
bool AimRage::TargetSpecificEnt(C_BasePlayer* pEnt)
{
	...
	if (g_Options.rage_lagcompensation)// && g_LocalPlayer->m_vecVelocity().Length2D() <= 0.1f && pEnt->m_vecVelocity().Length2D() > 0.1f) //backtrack moving players
	{
		if (tbt.joinable())
			tbt.join();
	}
	else
	{
		...
		if (g_Options.rage_autobaim && Global::iShotsFired > g_Options.rage_baim_after_x_shots || PlayerList[pEnt->EntIndex()].force_baim == 2)
			vecTarget = CalculateBestPoint(pEnt, HITBOX_PELVIS, g_Options.rage_mindmg, g_Options.rage_prioritize, matrix);
		else if (PlayerList[pEnt->EntIndex()].force_baim == 1)
			vecTarget = CalculateBestPoint(pEnt, HITBOX_UPPER_CHEST, g_Options.rage_mindmg, g_Options.rage_prioritize, matrix);
		else
