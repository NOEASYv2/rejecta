...
namespace Menu
{
	...
	void mainWindow()
	{
		...
		if (ImGui::Begin("Rejecta", &menuOpen, ImVec2(600, 400), 1.f, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_OnlyDragByTitleBar))
		{
			ImGui::BeginGroup();
	...
	void legitTab()
	{
		//no idea what is going on here... i would prefer a list box for the weapons
		...
		//kinda like iniuria maybe??

		//oh and do you know how to save/load arrays? cuz if not we have to make a switch with all weapons here....

		static char filterGuns[32];
		static int currWeap = 7;

		ImGui::Columns(2, NULL, false); {
			ImGui::BeginChild("##aa", ImVec2(0, 20), true); {
				ImGui::Text("Weapons");
				ImGui::EndChild();
			}
			ImGui::BeginChild("##ab", ImVec2(0, 0), true); {
				ImGui::PushItemWidth(-1);
				ImGui::InputText("##FilterGuns", filterGuns, 32);
				ImGui::ListBoxHeader("##GUNS", ImVec2(0, 15 * 19));
				...
				for (auto it : guns)
				{
					...
					if (find_insesitive_string(std::string(it.second), std::string(filterGuns)) && std::string(filterGuns).length() != 0)
						continue;
		...
		ImGui::NextColumn(); {
			ImGui::BeginChild("##ac", ImVec2(0, 20), true); {
				ImGui::Text("Settings");
				ImGui::EndChild();
			}
			ImGui::BeginChild("##ad", ImVec2(0, 0), true); {
				ImGui::Checkbox("Enable", &WeaponActive[currWeap]);

				ImGui::SliderFloat("Smoothing", &WeaponSmooth[currWeap], 0.f, 30.f, "%.1f", 1.f);

				ImGui::SliderFloat("Fov", &WeaponFOV[currWeap], 0.f, 10.f, "%.1f", 1.f);
				ImGui::EndChild();
			}
		}
	}
	...
	void rageTab()
	{
		...
		ImGui::NextColumn();
		{
			...
			{
				ImGui::Checkbox("Resolver", &g_Options.hvh_resolver);
	...
	void hvhTab() {
		...
		ImGui::NextColumn();
		{
			...
			ImGui::BeginChild("COL2", ImVec2(0, 0), true);
			{
				...
				ImGui::Combo("##AAA", &g_Options.hvh_antiaim_y_air, opt_AAair, 6);
				ImGui::Text("Fake Yaw");
				ImGui::Combo("##FAAY", &g_Options.hvh_antiaim_y_fake, opt_AAair, 6);
	...
	void miscTab()
	{
		ImGui::Columns(2, NULL, false);
		{
			ImGui::BeginChild("COL1", ImVec2(0, 100), true);
			{
				ImGui::Text("Movement");
				ImGui::Checkbox("Bunny Hop", &g_Options.misc_bhop);
				ImGui::Checkbox("Autostrafe", &g_Options.misc_autostrafe);
				ImGui::Checkbox("Circle Strafe", &g_Options.misc_circlestrafe);
				ImGui::NewLine();
				ImGui::KeyBindButton(&g_Options.misc_circlestrafe_bind);
				//ImGui::SliderFloat("Retrack", &g_Options.misc_circlestrafe_retrack, 0.5f, 10.0f);
				//ImGui::SliderFloat("Divisor", &g_Options.misc_circlestrafe_divisor, 0.5f, 10.0f);
				ImGui::EndChild();
			}
			ImGui::BeginChild("ha", ImVec2(0, 140), true);
			{
				ImGui::Text("Other");
				ImGui::Checkbox("Anti Untrusted", &g_Options.misc_antiut_proxy);
				ImGui::Checkbox("Legit Antiaim", &g_Options.misc_legitantiaim);
			...
			ImGui::BeginChild("hb", ImVec2(0, 0), true);
			{
				ImGui::Text("Remove");

				ImGui::Checkbox("Smoke", &g_Options.removals_smoke);
				ImGui::Checkbox("Flash", &g_Options.removals_flash);
				ImGui::Checkbox("Scope", &g_Options.removals_scope);
		...
		ImGui::NextColumn();
		{
			ImGui::BeginChild("COL2", ImVec2(0, 0), true);
			{
				...
				ImGui::Columns(2, NULL, true);
				{
					...
					if (ImGui::Button("Add", ImVec2(75, 20)))
					{
						char buf[MAX_PATH];
					...
					if (ImGui::Button("Remove", ImVec2(75, 20)))
					{
						if (configItems.size() > 0 && (configItemCurrent >= 0 && configItemCurrent < (int)configItems.size()))
						{
							char buf[MAX_PATH];
					...
					if (ImGui::Button("Refresh", ImVec2(75, 20)))
						configItems = ConfigZ::getAllConf();
				...
				ImGui::Checkbox("Bullet Tracers", &g_Options.misc_bullettracers);
				ImGui::ColorPickerBox3("##Bullets", g_Options.esp_manual_aa_indicator_color, false);
				ImGui::Combo("Type##Bullets", &g_Options.misc_bullettracers_type, opt_BulletTracerType, 2);
				ImGui::SliderFloat("Duration", &g_Options.misc_bullettracers_duration, 0.2f, 5.f);
				ImGui::SliderFloat("width", &g_Options.misc_bullettracers_width, 0.f, 10.f, "%.2f");
