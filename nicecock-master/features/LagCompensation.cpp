...
float CMBacktracking::GetNetworkLatency(int DIRECTION)
{
	// Get true latency
...
void CMBacktracking::RageBacktrack(C_BasePlayer* target, CUserCmd* usercmd, Vector &aim_point, bool &hitchanced)
{
	if (StartLagCompensation(target))
	{
		...
		while (FindViableRecord(target, &cur_record))
		{
			...
			if (g_Options.rage_autobaim && Global::iShotsFired > g_Options.rage_baim_after_x_shots || PlayerList[target->EntIndex()].force_baim == 2)
			...
			else if (PlayerList[target->EntIndex()].force_baim == 1)
				aim_point = AimRage::Get().CalculateBestPoint(target, HITBOX_UPPER_CHEST, g_Options.rage_mindmg, g_Options.rage_prioritize, iter->matrix);
			else
